...
global{
	...
	float LANDSCAPE_SIZE_LENTH <- 80.0 parameter: 'Greenhouse_lenth' category: 'Greenhouse';//20,80
	...
	int nb_clones_column <-390 parameter: 'Strawberry_column' category: 'Strawberry';//390,20
	//草莓种植行数
	...
	int nb_honybee_on_hive <- 8000 parameter: 'Honybee_on_hive' category: 'Honybee';
		...
		loop k from:0 to:(length(x_target.Flower) - 1){	
			if(x_target != nil){
				...
				//the flower is healthy
					...
					//Extract pollen 
					if(true){
						int extracted_pollen <- nb_pollen_extracted_per_visit;

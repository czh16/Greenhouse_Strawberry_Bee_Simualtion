...
global{
	...
	int simID <- 10 		parameter: 'simulationID' 	category: 'Simulation';
	...
	float LANDSCAPE_SIZE_LENTH <- 10.0 	parameter: 'Greenhouse_lenth' 	category: 'Greenhouse';//10,80
	//The strawberry number in a row
	int nb_clones_column <-12			parameter: 'Strawberry_column' 	category: 'Strawberry';//390,10
	//大棚中蜜蜂的数量，一箱意大利蜜蜂的数量是3500~4000左右，外勤工蜂比例是40%
	int nb_honybee_on_hive <- 100			parameter: 'Honybee_on_hive' 	category: 'Honybee';
	...
	action strawberryclone_growth (geometry geo_field_boundary) {		
		...
		loop i from:1 to:nb_clones_column{ 	
			loop i from:1 to:(nb_clones_column/4){ 	
			...
			loop i from:(nb_clones_column/4+1) to:(nb_clones_column/2){ 	
				loop j from:1 to:nb_clones_row{
					//每株草莓半径为0.1m, 垄间距0.4m+0.1m+0.1m 
					geometry tmp_geo1 <- circle(0.1) at_location {left_margin + 0.2*i, top_margin + 0.6*j} ;
					geometry tmp_geo2 <- circle(0.1) at_location {left_margin + 0.2*i, top_margin + 0.2 + 0.6*j};
			...
			loop i from:(nb_clones_column/2+1) to:(nb_clones_column/4*3){ 	
				loop j from:1 to:nb_clones_row{
					//每株草莓半径为0.1m, 垄间距0.4m+0.1m+0.1m 
					geometry tmp_geo1 <- circle(0.1) at_location {left_margin + 0.2*i, top_margin + 0.6*j} ;
					geometry tmp_geo2 <- circle(0.1) at_location {left_margin + 0.2*i, top_margin + 0.2 + 0.6*j};
			...
			loop i from:(nb_clones_column/4*3+1) to:(nb_clones_column){ 	
				loop j from:1 to:nb_clones_row{
					//每株草莓半径为0.1m, 垄间距0.4m+0.1m+0.1m 
					geometry tmp_geo1 <- circle(0.1) at_location {left_margin + 0.2*i, top_margin + 0.6*j} ;
					geometry tmp_geo2 <- circle(0.1) at_location {left_margin + 0.2*i, top_margin + 0.2 + 0.6*j};

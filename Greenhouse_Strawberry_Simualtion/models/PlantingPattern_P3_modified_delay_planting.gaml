...
global{
	string model_version <- "PlantingPattern_P3_modified_delay_planting";
	bool simulation_done <- false;
	//ID of the simulation experiment
	//编号11表示测试P2的延迟天数
	int simID <- 11 		parameter: 'simulationID' 	category: 'Simulation';
	//The repetition of the simulation experiment									
	int repeatID <- 1 	parameter: 'RepeatID' 		category: 'Simulation';
	int delay_day <- 0 	parameter: 'delay_day' 		category: 'Simulation';
	...
	int nb_honybee_on_hive <- 10000			parameter: 'Honybee_on_hive' 	category: 'Honybee';//10000
	//模拟持续的天数, 草莓实验中为110
	int simulation_duration <- int(max(columns_list(weather_data)[0]));
	//int simulation_duration <- 10;
	
	int planting_pattern <- 3			parameter: 'planting_pattern' 		category: 'Simulation';																				
	...
	int mean_gdd_primary_bloom_cultivar2 <- 586 + delay_day*5*15;	//草莓一级花序开花所需要的GDD均值, 586摄氏度，品种2
	int mean_gdd_secondary_bloom_cultivar1 <- 866;      		//草莓二级花序开花所需要的GDD均值, 1066摄氏度，修正为866
	int mean_gdd_secondary_bloom_cultivar2 <- 866 + delay_day*5*15; //草莓二级花序开花所需要的GDD均值, 1066摄氏度，修正为866
	...
	reflex stat_field when:simulation_started{
		...
		if((current_julian = simulation_duration)and(!save_done_2)){
			...
			loop c over:Clone_indices{
				save[simID, repeatID, planting_pattern,delay_day, 
			...
			*/
			//保存仿真参数和重要结果
			save [model_version, simID, repeatID, planting_pattern,delay_day,
		...
		*/
		...
		if(simulation_started and is_active) {
			loop f over:Honeybee_indices{
				...
				to:"../output/"+model_version + "_" +simID+"_"+repeatID+"_Honeybee_to_trace.csv" header:true type:"csv" rewrite:false;
			}		
		}
		*/
	}	
...
experiment Repetition type: gui{
	...
	reflex start_new_simulation when: simulation_done{
		...
		if(work_id <= total_repetition_works){
			//为了代码逻辑清晰，delay_day传入的数值分别为1和2，代表了延迟为1*5和2*10天
			create MainModel_model with: [delay_day::work_id];
		}
		else{
	...
	output{
		...
		//传入的延迟天数（虚假）
		monitor "delay_day:" 	value: delay_day;
		//真正的延迟天数
		monitor "delay_day_real:" 	value: delay_day*5;
	}
}

 ...
 * 	Planting Pattern 2 modified    
 */

model MainModel

global{
	string model_version <- "PlantingPattern_P2_modified";
	bool simulation_done <- false;
	//ID of the simulation experiment
	int simID <- 12 		parameter: 'simulationID' 	category: 'Simulation';
	//The repetition of the simulation experiment									
	int repeatID <- 2 	parameter: 'RepeatID' 		category: 'Simulation';
	int planting_pattern <- 2;
	...
	//草莓同不同品种亲和性
	float strawberry_cultivar_compatibility_probability <- 0.9 	parameter: 'Strawberry_cultivar_compatibility' 	category: 'Strawberry';
	...
	float ratio_bee_worker <- 0.5 				parameter: 'Ratio_bee_worker' 	category: 'Honybee';
	...
	float perception_distance <- 0.8 				parameter: 'Honeybee_perception_distance' category: 'Honybee';
	...
	reflex stat_field when:simulation_started{
		...
		if((current_julian = simulation_duration)and(!save_done_2)){
			...
			loop c over:Clone_indices{
				...
				to:"../output/"+model_version + "_" +simID+"_"+repeatID+"_Clone.csv" header:true format:"csv" rewrite:false; 
			}
			*/
			//保存仿真参数和重要结果
			save [model_version,simID, repeatID, planting_pattern,  
				...
				to:"../output/" +model_version + "_" + model_version + "_" +simID+"_"+repeatID+"_simulation_parameters_OnEnd.csv" header:true type:"csv" rewrite:false;
			...
			loop f over:Flower_indices{
				...
				to:"../output/"+model_version + "_" +model_version + "_" + simID + "_" + repeatID+"_Flower.csv" header:true type:"csv" rewrite:false;
			}
		...
		if(simulation_started and (cycle - growth_start_on) mod cycle_per_day = 0) {
				save [simID, repeatID, planting_pattern, 
					...
					to: (current_julian = (simulation_duration))?"../output/"+model_version + "_" +simID+"_"+repeatID+"_field_indicies_final.csv":"../output/"+simID+"_"+repeatID+"_field_indicies_EveryDay.csv" header:true type:"csv" rewrite:false;				
		}
		//实时保存跟踪的蜜蜂信息
...
experiment Repetition type: gui{
	...
	reflex start_new_simulation when: simulation_done{
		...
		if(work_id <= total_repetition_works){
			create MainModel_model with: [current_work_id::work_id,nb_honybee_on_hive::work_id + 10000];
		}
		else{
			write "All simulations are done!";
			ask MainModel_model{
				do pause();
			}
		}
	...
	output{
		monitor "Current day:" 	value: current_julian;
		monitor "current_work_id:" 	value: current_work_id;
	}
}

...
global{
	string model_version <- "0602_cn";
	bool simulation_done <- false;
	//ID of the simulation experiment
	int simID <- 4 		parameter: 'simulationID' 	category: 'Simulation';
	...
	float LANDSCAPE_SIZE_LENTH <- 20.0 	parameter: 'Greenhouse_lenth' 	category: 'Greenhouse';//10,80
	//The strawberry number in a row
	int nb_clones_column <-80			parameter: 'Strawberry_column' 	category: 'Strawberry';//390,10
	//大棚中蜜蜂的数量，一箱意大利蜜蜂的数量是3500~4000左右，外勤工蜂比例是40%
	int nb_honybee_on_hive <- 1000			parameter: 'Honybee_on_hive' 	category: 'Honybee';
	...
	action strawberryclone_growth (geometry geo_field_boundary) {		
		...
		loop i from:1 to:nb_clones_column{
			...
			/*  	
			loop i from:1 to:(nb_clones_column/2){ 	
				loop j from:1 to:nb_clones_row{
					//每株草莓半径为0.1m, 垄间距0.4m+0.1m+0.1m 
					geometry tmp_geo1 <- circle(0.1) at_location {left_margin + 0.2*i, top_margin + 0.6*j} ;
					geometry tmp_geo2 <- circle(0.1) at_location {left_margin + 0.2*i, top_margin + 0.2 + 0.6*j};
			...
			loop i from:(nb_clones_column/2+1) to:nb_clones_column{ 	
				loop j from:1 to:nb_clones_row{
					//每株草莓半径为0.1m, 垄间距0.4m+0.1m+0.1m 
					geometry tmp_geo1 <- circle(0.1) at_location {left_margin + 0.2*i, top_margin + 0.6*j} ;
					geometry tmp_geo2 <- circle(0.1) at_location {left_margin + 0.2*i, top_margin + 0.2 + 0.6*j};
...
experiment Repetition type: gui{
	...
	reflex start_new_simulation when: simulation_done{
		...
		if(work_id <= total_repetition_works){
			create MainModel_model with: [current_work_id::work_id];
		}
		else{

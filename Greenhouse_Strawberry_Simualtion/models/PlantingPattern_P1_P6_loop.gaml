 ...
 * 	P1-P6种植模式循环实验 
 *  直接点击 Repetition运行程序，依次循环P1—P6,循环结束需要手动关闭重新开始，否则会输出相同结果
 */

model MainModel

global{
	string model_version <- "PlantingPattern_P1_P6_loop";
	bool simulation_done <- false;
	//ID of the simulation experiment
	int simID <- 1 		parameter: 'simulationID' 	category: 'Simulation';
	//The repetition of the simulation experiment									
	int repeatID <- 9 	parameter: 'RepeatID' 		category: 'Simulation';
	int current_work_id <- 1;
	string batchID <- name;
	
	/*Greenhouse */
	//Length of the greenhouse
	float LANDSCAPE_SIZE_LENTH <- 80.0 	parameter: 'Greenhouse_lenth' 	category: 'Greenhouse';//10,80
	//The strawberry number in a row
	int nb_clones_column <- 390			parameter: 'Strawberry_column' 	category: 'Strawberry';//390,10
	//大棚中蜜蜂的数量，一箱意大利蜜蜂的数量是3500~4000左右，外勤工蜂比例是40%
	int nb_honybee_on_hive <- 10000			parameter: 'Honybee_on_hive' 	category: 'Honybee';
	//模拟持续的天数, 草莓实验中为110
	int simulation_duration <- int(max(columns_list(weather_data)[0]));
	//int simulation_duration <- 5;
	
	//六种种植模式
	int planting_pattern <- 1			parameter: 'planting_pattern' 		category: 'Simulation';
	...
	int ovules_in_third_inflorescence <-  180;	//三级花180个雌蕊，not used in this model
	
	/*Strawberry */
	//草莓种植行数
	int nb_clones_row 	 <-12 		parameter: 'Strawberry_row' 	category: 'Strawberry';//12
	//第一朵花开放的天数
	int bloom_day <- 0;
	...
	//草莓花期为5天
	int days_strawberry_bloom <- 6;  
	//控制花序等级，本实验中每朵花仅保留三个花序     
	float probability_of_clone_with_three_inflorescence <- 1.0;
	//草莓同品种亲和性
	float strawberry_self_compatibility_probability <- 0.9 		parameter: 'Strawberry_self_compatibility' 		category: 'Strawberry';
	//草莓同不同品种亲和性
	float strawberry_cultivar_compatibility_probability <- 0.95 	parameter: 'Strawberry_cultivar_compatibility' 	category: 'Strawberry';
	...
	float INIT_GDD <- 0.0;
	int mean_gdd_primary_bloom_cultivar1 <- 586;		//草莓一级花序开花所需要的GDD均值, 586摄氏度，品种1
	int mean_gdd_primary_bloom_cultivar2 <- 586;		//草莓一级花序开花所需要的GDD均值, 586摄氏度，品种2
	int mean_gdd_secondary_bloom_cultivar1 <- 866;      //草莓二级花序开花所需要的GDD均值, 1066摄氏度，修正为866
	int mean_gdd_secondary_bloom_cultivar2 <- 866;     //草莓二级花序开花所需要的GDD均值, 1066摄氏度，修正为866
	int sd_gdd <- 120;					                //草莓开花所需要的GDD方差 120
	int nb_pollen_perflower <- 450000;
	...
	float ratio_bee_worker <- 0.7 				parameter: 'Ratio_bee_worker' 	category: 'Honybee';
	...
	//蜜蜂一次外出觅食访问的花序数量
	int honeybee_workload <- 32 parameter: 'Honeybee_wordload' category: 'Honybee';
	...
	int honeybee_pollen_deposited_per_visit <- 35	parameter: 'Honeybee_pollen_deposited_per_visit' category: 'Honybee';
	int nb_traced_bee <- 1 							parameter: 'Honeybee_to_trace' category: 'Honybee';
	float perception_distance <- 1.0 				parameter: 'Honeybee_perception_distance' category: 'Honybee';
	...
	init {	
		...
		create Honeybee_indices number:(nb_traced_bee){}			
	}
	...
	//更新蜜蜂是否觅食, 为了节省内存, 必须在已经开花了, 并且果实没有全部成熟
	...
	//草莓种植方式的模型
	action strawberryclone_growth (geometry geo_field_boundary) {		
		...
		loop i from:1 to:nb_clones_column{ 	
			loop j from:1 to:nb_clones_row{
				...
				//P6
				if(planting_pattern = 6){
					if (i < nb_clones_column/4){
					...
					else if(i < nb_clones_column/4*3){
						create StrawberryClone number:1{
							shape <- tmp_geo1;
							cultivar <- 1;
						}
				...
				//P5 end
				//P4
				if(planting_pattern = 4){
					if (i <= nb_clones_column/2){
						create StrawberryClone number:1{
							shape <- tmp_geo1;
							cultivar <- 1;
						}
				...
				//P2 end
				//P1
				if(planting_pattern = 1){
					if(j <= nb_clones_row/2){
						create StrawberryClone number:1{
							shape <- tmp_geo1;
							cultivar <- 1;
						}
...
species Flower_indices {
	...
	//该花被蜜蜂访问的次数
	int Flower_bee_visit <- 0;
	int Flower_bee_visit_day1 <- 0;
	int Flower_bee_visit_day2 <- 0;
	int Flower_bee_visit_day3 <- 0;
	int Flower_bee_visit_day4 <- 0;
	int Flower_bee_visit_day5 <- 0;
	int Flower_bee_visit_success <- 0;
	
	//该花朵接受的本品种数量
	int Flower_nb_pollen_selfcultivar <- 0;
	//该花朵接受的异品种数量
	int Flower_nb_pollen_othercultivar <- 0;
	int Flower_nb_accepted_pollen <- 0;
	//草莓花所属的品种
	int Flower_of_cultivar <- 0;
}

species Honeybee_indices {
	string Honeybee_ID;
	point Honeybee_location;
	int Honeybee_bee_state;
	float Honeybee_bee_heading;
}
...
species Pollinator control:fsm skills:[moving] {
	...
	float azimuth;                 //方位角
	//传粉者的信息, 会被蜜蜂类覆盖
	int nb_pollen_deposited_per_visit <- honeybee_pollen_deposited_per_visit;	
	...
	species pollen_basket use_regular_agents: false use_individual_shapes: false{
		...
		int nb_pollen_grains <- 0;					//该篮子携带的花粉数量
		int cultivar_int <- 0;
	}

	reflex status_update when: is_active {
		nb_pollen_carring <- sum(pollen_basket collect each.nb_pollen_grains);  //所有篮子的花粉数，即蜜蜂携带的花粉数
		//if (nb_pollen_carring>0){write nb_pollen_carring;}
	}
	...
	state stay_nest initial: true{
		...
		exit{
			...
			//Find one of clone in bloom
				...
				//蜜蜂初始采蜜的花序
				//修复一个会导致仿真崩溃的bug
				//发生于一些花开的很早，蜜蜂进行活跃期，但是温室内没有合适的降落花序
				//将其随机放置于一朵花上
				if(!empty(Stem where (each.inflorescence_in_bloom = true))){
					location <- any(shuffle(Stem where (each.inflorescence_in_bloom = true) )).location;
				}
				else{
					location <- any(shuffle(Stem)).location;
				}
				//将盛开的花通过与蜂巢的距离进行排序
	...
	state visit_flower{	
		...
		if (target.flower_age = 1){target.nb_bee_visit_stem_day1 <- target.nb_bee_visit_stem_day1 + 1;}
		if (target.flower_age = 2){target.nb_bee_visit_stem_day2 <- target.nb_bee_visit_stem_day2 + 1;}
		if (target.flower_age = 3){target.nb_bee_visit_stem_day3 <- target.nb_bee_visit_stem_day3 + 1;}
		if (target.flower_age = 4){target.nb_bee_visit_stem_day4 <- target.nb_bee_visit_stem_day4 + 1;}
		if (target.flower_age = 5){target.nb_bee_visit_stem_day5 <- target.nb_bee_visit_stem_day5 + 1;}
		//Leave current stem
		target <- nil;	
	...
	action pollen_extacting (int xnb_pollen_grains, StrawberryClone x_genotype){
		...
		if(nb_pollen_carring < max_pollen_load){
			...
			list<pollen_basket> basket_indicator <- self.pollen_basket select (each.cultivar_int = x_genotype.cultivar);
			if(empty(basket_indicator)){
				...
				create pollen_basket number:1{
					...
					cultivar_int <- x_genotype.cultivar;
					//write cultivar_int;
					nb_pollen_grains <-  xnb_pollen_grains;
					//write "create pollen_basket with number: " + nb_pollen_grains;
				}
			}
			//If the pollen basket already has this type of pollen, then load them to the right place
				...
				//write "supplement pollen: " + xnb_pollen_grains;
			}
			//write "pollen_extacting stage: " + pollen_basket count (each.nb_pollen_grains > 0);			
		}
	}
	...
	action target_detect {
		//target是花序，可以显性显示
		target <- shuffle(Stem at_distance perception_distance - visit_history) first_with (each.flower_state = 1);
	}
	...
	action pollinator_stem_contact (Stem x_target, int xnb_visits) type: int{
		...
		loop k from:0 to:(length(x_target.Flower) - 1){	
			if(x_target != nil){
				int i <- k;
				x_target.Flower[i].nb_bee_visit_stem_success <- x_target.Flower[i].nb_bee_visit_stem_success + 1;	
				...
				if((x_target.Flower[i].nb_received_pollen < x_target.stigma_capacity) and nb_pollen_types > 0 and nb_pollen_carring > 0){
					...
					//write "nb_actual_pollens  " + nb_actual_pollens;
					loop p over:pollen_basket{
						//真正交互的花粉数*本花粉篮子中的花粉数/本蜜蜂携带的花粉数，把每个篮子中的花粉平均卸载
						int nb_deposited <- int(nb_actual_pollens * p.nb_pollen_grains / nb_pollen_carring) + 1;
						//write "nb_deposited" + nb_deposited;
						p.nb_pollen_grains <- p.nb_pollen_grains - nb_deposited;					
						bool pollen_compatibility;
						//同品种草莓花授粉
						if(p.cultivar_int = x_target.host_clone.cultivar){
							//自花授粉的情况
							pollen_compatibility <- flip(x_target.host_clone.self_pollen_compatibility);
							//write "同品种"+pollen_compatibility;
						}
						else{
							//异品种草莓花授粉        
							pollen_compatibility <- flip(x_target.host_clone.cultivar_pollen_compatibility);
							//write "异品种"+pollen_compatibility;
						}
						//Check pollen compatibility and receptivity
						//If it is receptable
						//花粉必须先通过receptivity, 再通过compatibility
						//write "x_target.flower_age:" + x_target.flower_age;
						//write "x_target.pollen_receptivity" + x_target.pollen_receptivity;
						if(flip(x_target.pollen_receptivity)){			//pollen_receptivity是收到柱头的影响
							...
							if(pollen_compatibility){
								...
								//如果是同品种授粉
								if(p.cultivar_int = x_target.host_clone.cultivar){ 
									x_target.Flower[i].nb_pollen_selfcultivar <- x_target.Flower[i].nb_pollen_selfcultivar + nb_deposited;
								}
								//如果是异品种授粉
								else { 
									x_target.Flower[i].nb_pollen_othercultivar <- x_target.Flower[i].nb_pollen_othercultivar + nb_deposited;
								}
							}
							...
							//write "pollen_receptivity is unsatisfied";
						}
				...
				if(x_target.Flower[i].nb_gen_pollen >= 0 and x_target.Flower[i].fruit_mass >= 0){
					...
					//加上条件，限制当花粉剩余数量小于0时，固定为0
					x_target.Flower[i].nb_gen_pollen <- max([x_target.Flower[i].nb_gen_pollen, 0]);
					//A successful visit
...
species Honeybee parent:Pollinator schedules:shuffle(Honeybee){
	...
	int workload <- honeybee_workload;
...
species Stem use_regular_agents: false use_individual_shapes: false schedules: [] {
	...
	//花朵的状态, 0发芽期绿色, 1开花期黄色, 2果实发育期粉色, 3果实成熟期红色
	int flower_state <- 0;  
	//花龄
	int flower_age <- 0;                                                  
	//花粉的接受概率
	...
	int nb_bee_visit_stem_day1 <- 0;
	int nb_bee_visit_stem_day2 <- 0;
	int nb_bee_visit_stem_day3 <- 0;
	int nb_bee_visit_stem_day4 <- 0;
	int nb_bee_visit_stem_day5 <- 0;
	...
	species Flower use_regular_agents: false use_individual_shapes: false schedules: []{
		...
		//18000 * 25 = 450000
		int nb_gen_pollen <- nb_pollen_perflower;						//每朵花的花粉粒总数
		int nb_fertilized_ovules <- 0;                                    //受孕的胚珠数量，=花接受的花粉粒数量
		int nb_achenes <- 0;											  //瘦果数量，=受孕的胚珠数量
		float fruit_mass <- 0.0;                                          //根据可育种子数估算的浆果质量
		int nb_pollen_selfcultivar <- 0;  
		int nb_pollen_othercultivar <- 0;
		int nb_bee_visit_stem_success <- 0;
...
species StrawberryClone {
		...
		loop i from:0 to:nb_stems-1{
			create Stem number:1 returns: new_stem{							
				...
				if (i = 0){
					...
					//一级花开花所需要的的GDD,different cultivars have different GDD
					if (myself.cultivar = 1){
						inflorescence_bloom_heat <- int(gauss(mean_gdd_primary_bloom_cultivar1, sd_gdd));
					}
					else if (myself.cultivar = 2){
						inflorescence_bloom_heat <- int(gauss(mean_gdd_primary_bloom_cultivar2, sd_gdd));
					}
				...
				//Secondary inflorescence
					...
					//二级花开花所需要的的GDD
					if (myself.cultivar = 1){
						inflorescence_bloom_heat <- int(gauss(mean_gdd_secondary_bloom_cultivar1, sd_gdd));
					}
					else if (myself.cultivar = 2){
						inflorescence_bloom_heat <- int(gauss(mean_gdd_secondary_bloom_cultivar2, sd_gdd));
					}   							
					inflorescence_berry_ripeness_heat <- int(gauss(mean_gdd_ripeness, sd_gdd_ripeness));
					//二级花序柱头花粉容纳上限是260
	...
	reflex stem_update when:current_julian > START_JULIAN and ((cycle - growth_start_on) mod cycle_per_day = 0){
		loop s over: list_stems{
				...
				s.pollen_receptivity <- exp(-0.01 * s.flower_age^3.6);         				         
	...
	action fertilization_checking (Stem xStem){	
 	...
 	// 给每一个草莓植株赋予不同的自花适应概率
	action init_pollen_compatibility{
		 self_pollen_compatibility <- strawberry_self_compatibility_probability;
		 cultivar_pollen_compatibility <- strawberry_cultivar_compatibility_probability;
	}					
	// 草莓植株的绘制	
	aspect my_aspect {
		// 品种1使用黑色圆圈
		if(cultivar = 1){
			//Display color for different cultivar
			draw shape.contour color: simulation_started?°black:°white;
		}
		// 品种2使用蓝色圆圈
		else if (cultivar = 2){
			rgb color2 <- rgb (210,210,210);
			draw shape.envelope color: simulation_started?color2:°white;
			draw shape.contour color: simulation_started?°black:°white;
		}
...
species SystemMonitor parallel:false use_regular_agents: false use_individual_shapes: false {	
	...
	//reflex update_clone_indices  when:simulation_started{
		...
		loop i from:0 to:length(StrawberryClone) - 1{
				...
				//
				Clone_indices[i].clone_of_cultivar <- StrawberryClone[i].cultivar;
				//草莓的坐标
				...
				loop s from:0 to:2{
					loop f from:0 to:(StrawberryClone[i].list_stems[s].nb_flowers_in_inflorescence-1) {
						...
						//记录该花所属的草莓品种
						Flower_indices[flower_nb].Flower_of_cultivar <- StrawberryClone[i].cultivar; 
						//记录花朵的花序等级
						...
						Flower_indices[flower_nb].Flower_bee_visit_day1 <- StrawberryClone[i].list_stems[s].nb_bee_visit_stem_day1;
						Flower_indices[flower_nb].Flower_bee_visit_day2 <- StrawberryClone[i].list_stems[s].nb_bee_visit_stem_day2;
						Flower_indices[flower_nb].Flower_bee_visit_day3 <- StrawberryClone[i].list_stems[s].nb_bee_visit_stem_day3;
						Flower_indices[flower_nb].Flower_bee_visit_day4 <- StrawberryClone[i].list_stems[s].nb_bee_visit_stem_day4;
						Flower_indices[flower_nb].Flower_bee_visit_day5 <- StrawberryClone[i].list_stems[s].nb_bee_visit_stem_day5;
						Flower_indices[flower_nb].Flower_bee_visit_success <- StrawberryClone[i].list_stems[s].Flower[f].nb_bee_visit_stem_success;
						//记录每朵花来自同品种和异品种的花粉数量
						Flower_indices[flower_nb].Flower_nb_pollen_selfcultivar <- StrawberryClone[i].list_stems[s].Flower[f].nb_pollen_selfcultivar;
						Flower_indices[flower_nb].Flower_nb_pollen_othercultivar <- StrawberryClone[i].list_stems[s].Flower[f].nb_pollen_othercultivar;
						Flower_indices[flower_nb].Flower_nb_accepted_pollen <- StrawberryClone[i].list_stems[s].Flower[f].nb_accepted_pollen;
						/* 
						...
						*/					
						...
						//Flower_beery_malformed 记录花朵未生长的果实，状态标记为0
	...
	//实时更新蜜蜂的信息
	reflex update_bee_indices  when:simulation_started and flower_in_bloom{
		//loop j from:0 to:length(Honeybee) - 1{
		//蜜蜂数量太多，只更新其中两只作为代表
		loop i from:0 to:nb_traced_bee-1 {
			Honeybee_indices[i].Honeybee_ID <- Honeybee[i].name;
			Honeybee_indices[i].Honeybee_location <- Honeybee[i].location;
			Honeybee_indices[i].Honeybee_bee_state <- Honeybee[i].bee_state;
			Honeybee_indices[i].Honeybee_bee_heading <- Honeybee[i].heading;
		}
	}
	...
	reflex stat_field when:simulation_started{
		...
		if((current_julian = simulation_duration)and(!save_done_2)){
			...
			loop c over:Clone_indices{
				save[simID, repeatID, planting_pattern, 
					c.clone_ID,c.clone_of_cultivar, c.clone_size, c.clone_location_x, c.clone_location_y,c.clone_avg_beevisits_stem, c.clone_fruitset_mean, c.clone_fruitset_std, c.clone_fruitsetII_mean, c.clone_fruitsetII_std,
				...
				to:"../output/"+model_version + "_" +simID+"_"+repeatID+"_Clone.csv" header:true type:"csv" rewrite:false; 
			}
			*/
			//保存仿真参数和重要结果
			save [model_version, simID, repeatID,planting_pattern, 
				  ...
				  nb_honybee_on_hive, ratio_bee_worker, honeybee_workload, honeybee_max_travel_dist,
				  avg_fruit_mass,percent_malformed_berries
				]
				to:"../output/"+model_version + "_" +simID+"_"+repeatID+"_simulation_parameters_OnEnd.csv" header:true type:"csv" rewrite:false;
			/* 
			//保存花朵信息
			loop f over:Flower_indices{
				save[simID, repeatID, planting_pattern, 
					 f.Flower_ID, f.Flower_of_cultivar, f.Flower_in_clone_ID,f.Flower_in_inflorescence_ID,
					 f.Flower_bloom_day,f.Flower_ripeness_day,
					 f.Flower_fruit_mass,f.Flower_beery_malformed,f.Flower_bee_visit,f.Flower_bee_visit_day1,f.Flower_bee_visit_day2,f.Flower_bee_visit_day3,f.Flower_bee_visit_day4,f.Flower_bee_visit_day5,f.Flower_bee_visit_success,
					 f.Flower_nb_pollen_selfcultivar,f.Flower_nb_pollen_othercultivar,f.Flower_nb_accepted_pollen] 
				to:"../output/"+model_version + "_" +simID+"_"+repeatID+"_Flower.csv" header:true type:"csv" rewrite:false;
			}
			*/	
			save_done_2 <- true;
		}
		...
		//实时保存跟踪的蜜蜂信息
		if(simulation_started and is_active) {
			loop f over:Honeybee_indices{
				//write f.Honeybee_bee_heading; 
				save[simID, repeatID, planting_pattern,
					 cycle, current_julian, time_8_to_17, time_minute,
					 f.Honeybee_ID,f.Honeybee_location.x,f.Honeybee_location.y,f.Honeybee_bee_state] 
				to:"../output/"+simID+"_"+repeatID+"_Honeybee_to_trace.csv" header:true type:"csv" rewrite:false;
			}		
		}
		*/
	}	
...
//重复N次实验，可以运行，不要改动
experiment Repetition type: gui{
	//目前运行的实验轮次
	int work_id <- 1;
	int total_repetition_works <- 6;
	init {
		MainModel_model[0].simulation_done <- true;
	}
	//只有每次实验结束后才运行以下代码
	reflex start_new_simulation when: simulation_done{
		//如果没有达到N次实验，则创建下一次实验
		if(work_id <= total_repetition_works){
			create MainModel_model with: [current_work_id::work_id,planting_pattern::work_id];
		}
		else{
			write "All simulations in this epoch are done!";
			//ask MainModel_model{
				//do pause();
			//}
		}
	...
	output{
		...
		monitor "planting_pattern:" 	value: current_work_id;
	}
}